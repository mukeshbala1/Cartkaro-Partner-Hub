...
class _RestaurantRegistrationScreenState
  ...
  final _minOrderCtrl             = TextEditingController(text: '149');
  ...
  Future<void> _saveDraft() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('registration_drafts').doc('${user.uid}_restaurant').set(draftData);
    } catch (e) {
  ...
  Future<void> _loadDraft() async {
    ...
    if (user == null) return;
    try {
      final doc = await FirebaseFirestore.instance.collection('registration_drafts').doc('${user.uid}_restaurant').get();
      if (doc.exists && doc.data() != null) {
        ...
        setState(() {
          ...
          _acceptTableOrders = data['acceptTableOrders'] ?? true;
          _dineInAvailable = data['dineInAvailable'] ?? true;
          ...
          _minOrderCtrl.text = data['minOrder'] ?? '149';
          _estDeliveryCtrl.text = data['estDelivery'] ?? '30 - 45 mins';
      ...
      if (nextPage == 8) {
        ...
        try {
            ...
            if (mounted) {
              showDialog(
                ...
                builder: (ctx) => AlertDialog(
                  ...
                  content: const Text("Please log in with your mobile number to complete restaurant registration."),
                  actions: [
                    ElevatedButton(
                      onPressed: () {
                        Navigator.pop(ctx);
          ...
          final businessData = {
            ...
            'storeAddress': _restaurantAddressCtrl.text.trim(),
            'fullAddress': _restaurantAddressCtrl.text.trim(),
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('🎉 Restaurant registered successfully! Awaiting Admin Approval.'),
                backgroundColor: Color(0xFF0284C7),
          ...
          if (mounted) {
            showDialog(
              ...
              builder: (ctx) => AlertDialog(
                ...
                content: Text("Could not save your restaurant data to Firebase.\n\nError: $e\n\nPlease check your internet connection and try again."),
                actions: [
                  TextButton(
...
class _PrefilledMobileField extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    String displayMobile = (mobile != null && mobile!.trim().isNotEmpty) ? mobile!.trim() : '';
    if (displayMobile.isEmpty) {
      final user = FirebaseAuth.instance.currentUser;
      if (user != null && user.phoneNumber != null && user.phoneNumber!.isNotEmpty) {
        displayMobile = user.phoneNumber!.replaceAll('+91', '').trim();
      }
    }
    if (displayMobile.isEmpty) {
      displayMobile = '5555555555';
    }
