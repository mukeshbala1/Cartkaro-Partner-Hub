...
class _GroceryRegistrationScreenState
  ...
  final _minOrderCtrl        = TextEditingController(text: '199');
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        ...
        setState(() {
          ...
          _minOrderCtrl.text = data['minOrder'] ?? '199';
  ...
  void dispose() {
    ...
    for (final c in [
      ...
      _altMobileCtrl, _storeNameCtrl, _storeAddressCtrl, _areaCtrl, _cityCtrl,
  ...
  Future<void> _nextStep() async {
    ...
    if (_currentStep == 1) {
      if (_storeLogoPath.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Please upload your Store Logo"),
            backgroundColor: Color(0xFFDC2626),
          ),
        );
        return;
      }
      if (_storeBannerPath.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Please upload your Store Banner"),
            backgroundColor: Color(0xFFDC2626),
          ),
        );
        return;
      ...
      if (_storePhotos.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Please upload at least one Store Photo"),
            backgroundColor: Color(0xFFDC2626),
          ),
        );
        return;
        ...
        try {
          ...
          if (effectiveUid.isNotEmpty) {
            try {
              ...
              if (existingGrocery.isNotEmpty) {
                final existingId = existingGrocery.first.id;
                await AuthService.saveActiveBusinessId(existingId);
                await AuthService.saveBusinessType('grocery');
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('You have already registered a Grocery Store under this account. Opening dashboard...'),
  ...
  Widget _buildStep2() {
    return _StepWrapper(
      ...
      children: [
        ...
        _SectionLabel(label: 'Store Location on Map & GPS'),
        ...
        const SizedBox(height: 20),
        _SectionLabel(label: 'Store Address Details (Auto-filled)'),
        CustomTextField(
        ...
        const SizedBox(height: 20),
        _SectionLabel(label: 'Store Branding (Required) *'),
